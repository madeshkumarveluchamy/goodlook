...
import './css/TiltTurnShow.css'; // 🎯 பிக்ஸ்: சிஎஸ்எஸ் ஃபைல் பெயர் மாற்றப்பட்டுள்ளது
...
const TiltTurnShow = () => {
  ...
  return (
    <section className="tiltturn-show-section py-5 overflow-hidden">
      <div className="container px-md-4">
        
        <div 
          className="tiltturn-show-track-window"
          ref={scrollRef}
        ...
        >
          <div className="tiltturn-show-inner-flex">
            {rodData.map((item, index) => (
              <div key={`${item.id}-${index}`} className="tiltturn-show-item-col">
                <div className="tiltturn-show-product-card">
                  
                  <div className="tiltturn-show-image-wrapper">
                    <img 
                      src={item.image} 
                      alt={`${item.title} Premium curtains collections`} 
                      className="tiltturn-show-product-img"
                      draggable="false" 
                    />
                  </div>

                  <div className="tiltturn-show-info-box-overlay">
                    <span className="tiltturn-show-brand-badge manrope-font ssub">{item.badge}</span>
                    <p className="tiltturn-show-product-title m-0 font-serief sdes">{item.title}</p>
                  </div>
        ...
        {/* Buttons */}
        <div className="tiltturn-slider-navigation-wrapper">
          <button className="tiltturn-nav-btn tiltturn-btn-prev" onClick={handlePrev} aria-label="Previous slide">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
            </svg>
          </button>
          <button className="tiltturn-nav-btn tiltturn-btn-next" onClick={handleNext} aria-label="Next slide">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
